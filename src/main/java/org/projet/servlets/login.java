...
public class login extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            Personne personne = loginDB.checkLogin(email, password);
            ...
            if (personne != null) {
                ...
                session.setAttribute("DT_NAISSANCE", personne.getNOM());
                session.setAttribute("EMAIL", personne.getEMAIL());
                session.setAttribute("STATUT", personne.getSTATU());
                session.setAttribute("Isloggedin", "yes");
                session.setAttribute("todaydate",dtf.format(now));
                if(personne.getCHANGED()==0){
                    destPage = "changerMDP.jsp";
                }else{
                    destPage = "index.jsp";
                }
            } else {
