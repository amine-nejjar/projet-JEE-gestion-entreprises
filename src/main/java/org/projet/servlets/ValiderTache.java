...
public class ValiderTache extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        TacheDB.valider(id, comment);
        
        Notification notif=new Notification();
        HttpSession session = request.getSession();
        String notification=(String) session.getAttribute("NOM")+" || tache en attente";
        notif.setID_SOURCE((String) session.getAttribute("CIN"));
        notif.setID_DESTINATION(EmployeDB.getIdChef((String) session.getAttribute("CIN")));
        notif.setMESSAGE(notification);
        NotificationDB.inserer(notif);

        session.setAttribute("msgValid","Success");
        String redirectURL = "ValiderTache.jsp";
